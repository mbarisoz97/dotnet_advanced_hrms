...
public class TrainingRecommendationControllerPutTests : TrainingManagementBaseIntegrationTest
{
    private const string RecommendationPreferenceEndpoint = "RecommendationPreferences";

    public TrainingRecommendationControllerPutTests(TrainingManagementWebApplicationFactory factory) : base(factory)
    {
    }
    ...
    public async Task Post_ExistingTrainingIdWithValidDetails_ReturnsOkWithReadRecommendationPreferenceDto()
    {
        ...
        var response = await client.PutAsJsonAsync($"{Endpoints.TrainingRecommendationApi}/{RecommendationPreferenceEndpoint}", command);
